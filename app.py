...
def calculate_fibonacci_levels(ticker, start_date, end_date, calculation_type="daily"):
    try:
        ...
        if calculation_type == "weekly" or calculation_type == "monthly":
            ...
            # print(f"Resampling data to {resample_text} frequency...") # Optional: for server logs
            ...
            # print(f"{calculation_type.capitalize()} resampling complete.") # Optional: for server logs
        else: # Daily
            df_input = processed_data[['Date', 'High', 'Low']].copy()
            # print("Using daily data.") # Optional: for server logs
